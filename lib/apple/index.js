...
function parseResult(result) {
    ...
    if (result.hasOwnProperty('latest_receipt_info') && result.latest_receipt_info[0]) {
        latestReceiptInfo = result.latest_receipt_info.sort(function (a, b) {
            a = parseInt(a.original_purchase_date_ms);
            b = parseInt(b.original_purchase_date_ms);

            return a - b;
        });

        productId = latestReceiptInfo.slice(-1).pop().product_id;
        transactionId = latestReceiptInfo.slice(-1).pop().transaction_id;
    }
