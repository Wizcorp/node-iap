...
function parseResult(result) {
	...
	var expirationDate = parseInt(getReceiptFieldValue(result.receipt, 'expires_date_ms'));
	...
	if (result.hasOwnProperty('latest_receipt_info')) {
		if(Array.isArray(result.latest_receipt_info) && result.latest_receipt_info.length > 0) {
			...
			purchaseData = lastReceipt.purchase_date_ms;
			expirationDate = lastReceipt.expires_date_ms;
			...
			purchaseData = result.latest_receipt_info.purchase_date_ms;
			expirationDate = result.latest_receipt_info.expires_date_ms;
		}
	}
