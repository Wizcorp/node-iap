...
function parseResult(result) {
	...
	var purchaseData = parseInt(getReceiptFieldValue(result.receipt, 'purchase_date_ms'));
	var expirationDate = parseInt(getReceiptFieldValue(result.receipt, 'expires_date'));
	...
	if (result.hasOwnProperty('latest_receipt_info')) {
		if(Array.isArray(result.latest_receipt_info) && result.latest_receipt_info.length > 0) {
			...
			purchaseData = parseInt(lastReceipt.purchase_date_ms);
			expirationDate = parseInt(lastReceipt.expires_date);
			...
			purchaseData = parseInt(result.latest_receipt_info.purchase_date_ms);
			expirationDate = parseInt(result.latest_receipt_info.expires_date);
		}
	}
