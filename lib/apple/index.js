...
function parseResult(result) {
	...
	var expirationDate = parseTime(getReceiptFieldValue(result.receipt, 'expires_date'));
	...
	if (result.hasOwnProperty('latest_receipt_info')) {
		if (Array.isArray(result.latest_receipt_info) && result.latest_receipt_info.length > 0) {
			...
			expirationDate = parseTime(lastReceipt.expires_date);
			...
			expirationDate = parseTime(result.latest_receipt_info.expires_date);
		}
	}
